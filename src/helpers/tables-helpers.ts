 ...
 */
export function setColumnsUserConfig(
    totalColumns: number,
    config: Record<string, 'left' | 'center' | 'right'>
): Record<number, { alignment: 'left' | 'center' | 'right' }> {
    const columns: Record<number, { alignment: 'left' | 'center' | 'right' }> = {};
...
export function printCoverageSummary(result: CoverageResult, tableOutput: string, minCoverage: number, core: any) {
      let summary = `\n** 📑 Resumo:**\n`;
      ...
      if (result.lineCoverage < minCoverage) {
        core.setFailed(
          `Line coverage ${result.lineCoverage.toFixed(
            2
          )}% is below the minimum threshold of ${minCoverage}%`
        );
        return;
      }
      if (result.branchCoverage < minCoverage) {
        core.setFailed(
          `Branch coverage ${result.branchCoverage.toFixed(
            2
          )}% is below the minimum threshold of ${minCoverage}%`
        );
        return;
