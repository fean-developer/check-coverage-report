...
import { CoverageResult, CoverageElement } from '../interfaces/coverage-result';
...
export function parseCoverageReport(xml: string): CoverageResult {
    ...
    function extractCounters(node: any) {
        const counters = node.counter ? (Array.isArray(node.counter) ? node.counter : [node.counter]) : [];
        const get = (type: string, attr: string) => {
            const c = counters.find((c: any) => c['@_type'] === type);
